...
- `app/composables/render/aircraft/tracks.ts` owns client turns fetching and rendering; its per-CID turns cache must be invalidated when `flightPlanTime` changes (a reconnect/new online flight). It preserves rendered history across temporary turns failures.
...
- `app/utils/server/vatsim/update.ts` normalizes and enriches live VATSIM data, including pilot status, routes, transceivers, achievements, sectors, bookings, tracks, and websocket counters.
  ...
  - Event ingestion in `app/utils/server/tasks.ts` normalizes the IVAO `v1/events` payload through `app/utils/server/vatsim/events.ts` into the existing `VatsimEvent` shape, derives short descriptions, and keeps the next 28 days in Redis.
